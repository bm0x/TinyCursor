...
use crate::platform::windows::{
    ...
    sample_screen_luminance, CursorDesignOverride, CursorSurface, OverlayWindow,
    RenderCursorKind, SystemCursorGuard, ThemeMode, TrayManager,
};
#[cfg(target_os = "windows")]
...
#[cfg(target_os = "windows")]
fn run_windows() {
    // 0. Enable Windows 1ms high-precision timer for ultra-smooth high-Hz tracking
    unsafe {
        timeBeginPeriod(1);
    }
    ...
    let mut physics = SmoothCursorPhysics::new(initial_pos, config);

    // 3. Detect initial screen luminance and initialize theme blend state
    let initial_lum = sample_screen_luminance(initial_pos);
    let mut target_color_blend = if initial_lum > 130.0 { 1.0f32 } else { 0.0f32 };
    let mut color_theme_blend = target_color_blend;

    // 4. Create transparent click-through overlay window
    ...
    // 5. Create 32-bit ARGB surface
    let mut surface = match CursorSurface::new() {
        Some(s) => s,
        None => {
            unsafe { timeEndPeriod(1); }
            return;
    ...
    // 6. Hide system cursors with RAII fail-safe protection
    let mut cursor_guard = SystemCursorGuard::hide();
    ...
    let target_fps = (display_hz * 2).clamp(120, 500); // 288 Hz for 144 Hz display
